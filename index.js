...
const runAction = () => {
	...
	const useYarn = existsSync(yarnPkgLockPath);
	log(`Will run ${useYarn ? "Yarn" : useNpm ? "NPM" : "PNPM"} commands in directory "${pkgRoot}"`);
	...
	run(useYarn ? "yarn" : useNpm ? "npm install" : "pnpm install", pkgRoot);
